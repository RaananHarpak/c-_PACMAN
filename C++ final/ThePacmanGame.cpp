...
int ThePacmanGame::run(fstream& stepfile,fstream& resfile,int score, int totalPoints, char level,int caseOpt,int& totalPacmanMoves)
{
	...
	do {
		...
		if (checkWin(key2, score, totalPoints,caseOpt) == true)
			return WON;
		p.move(NextPacmanChar, NextPacmanStep, IfMenuIsFirst(), rowBoard, colBoard, stepfile,"Pacman ",withFiles,totalPacmanMoves,caseOpt);
		UpdateSteps(CurrentPacmanStep, CurrentFruitStep, NextPacmanStep, NextFruitStep, CurrentPacmanChar, NextPacmanChar, CurrentFruitChar, NextFruitChar, CurrentGhostsCharArr, NextGhostsCharArr,p, fruit, ghostCounter, CurrentGhostsStepArr, NextGhostsStepArr, gstarr, PACdir);
		...
		if (delaymonsters == false)//Delay the monster and move the ghosts:
		{
			fruitCounterRun++;
			if (visibleFruit == true)
			randomMade = fruit.move(NextFruitChar,CurrentFruitChar, IfMenuIsFirst(), rowBoard, colBoard, stepfile, "Fruit ", withFiles,caseOpt);
			while (randomMade == true)
			{
				UpdateSteps(CurrentPacmanStep, CurrentFruitStep, NextPacmanStep, NextFruitStep, CurrentPacmanChar, NextPacmanChar, CurrentFruitChar, NextFruitChar, CurrentGhostsCharArr, NextGhostsCharArr, p, fruit, ghostCounter, CurrentGhostsStepArr, NextGhostsStepArr, gstarr, PACdir);
				randomMade = fruit.move(NextFruitChar, CurrentFruitChar, IfMenuIsFirst(), rowBoard, colBoard, stepfile, "Fruit ", withFiles,caseOpt);
			...
			{
				string obj = "Ghost " + std::to_string(i)+" ";
				randomMade = gstarr[i].move(NextGhostsCharArr[i], CurrentGhostsCharArr[i], IfMenuIsFirst(), level, rowBoard, colBoard, stepfile,obj,withFiles,caseOpt);
				while (randomMade == true)
				{
					UpdateSteps(CurrentPacmanStep, CurrentFruitStep, NextPacmanStep, NextFruitStep, CurrentPacmanChar, NextPacmanChar, CurrentFruitChar, NextFruitChar, CurrentGhostsCharArr, NextGhostsCharArr, p, fruit, ghostCounter, CurrentGhostsStepArr, NextGhostsStepArr, gstarr, PACdir);
					randomMade = gstarr[i].move(NextGhostsCharArr[i], CurrentGhostsCharArr[i], IfMenuIsFirst(), level, rowBoard, colBoard, stepfile, obj, withFiles,caseOpt);
...
int ThePacmanGame::runFromFile(fstream& stepfile, fstream& resfile, int score, int totalPoints, char level, int caseOpt, int& totalPacmanMoves)
{
	...
	do {
		...
		if (ifContinue == true)
		{
			//no menu at load or silent mode
			/*	if (!silentMood)
			{
				if (IfMenuIsFirst() == true)
				{
			...
			if (delaymonsters == false)//Delay the monster and move the ghosts:
			{
				fruitCounterRun++;
				if (visibleFruit == true)
					fruit.move(NextFruitChar, CurrentFruitChar, IfMenuIsFirst(), rowBoard, colBoard, stepfile, "Fruit ", withFiles,caseOpt);
				CurrentPacmanStep = Point(p.getx(), p.gety());
				CurrentFruitStep = Point(fruit.Point::getx(), fruit.Point::gety());
				CalcCurrentGhostsSteps(ghostCounter, CurrentGhostsStepArr, gstarr);
				CalcNextGhostsStep(ghostCounter, NextGhostsStepArr, gstarr);
				...
				{
					string obj = "Ghost " + std::to_string(i) + " ";
					gstarr[i].move(NextGhostsCharArr[i], CurrentGhostsCharArr[i], IfMenuIsFirst(), level, rowBoard, colBoard, stepfile, obj, withFiles,caseOpt);
				}
				delaymonsters = true;
				CurrentPacmanStep = Point(p.getx(), p.gety());
				CurrentFruitStep = Point(fruit.Point::getx(), fruit.Point::gety());
...
void FindStepsFiles(int numOfScreenFiles) {
	int counter1 = 0, counter2 = 0;
	string exceptionmsg="Steps file is missing for run a game from files.   You should have number of steps files as screen files";
	string extension1 = ".steps";
	fs::path path = fs::current_path();
	for (auto& p : fs::recursive_directory_iterator(path)) {
...
int ThePacmanGame::StartGame(fstream& inputfile,fstream& stepfile,fstream& resfile, ThePacmanGame& p, char level,int caseOpt, int& totalPacmanMoves)
{
	...
	for (int i = 0; i < rowBoard; i++)
	{
		...
		else if (caseOpt == 2)
		{
			...
			if (res == WON)
			{
				resfile << "Won after:" << totalPacmanMoves << " moves." << endl;
				return WON;
			}
		...
		{
			res = p.runFromFile(stepfile, resfile, 0, totalPoints, level, caseOpt, totalPacmanMoves); 
			if (res == LOSE)
			return LOSE;
		...
		else if (caseOpt == 4)
		{
			...
			if (res == LOSE)
			{
				...
				getline(resfile, resultfromfile);
				bool match = CompareResults(resultfromfile,res, totalPacmanMoves);
				if (match==false)
			...
			if (res == WON)
			{
				...
				cout << "Press any key to go to continue" << endl;
				char key = _getch();
				return WON;
			}
...
bool ThePacmanGame::CompareResults(string resFromFile,int res,int totalScore)
{//"Loose after:" << totalPacmanMoves << " moves.
	int index = 0;
	string sub_str = "";
	int number = 0;
	string resStr;
	...
	while (resFromFile[index] != ' ')
	{
		sub_str += resFromFile[index++];
	}
	resStr = sub_str;
	...
	index++;
	while (isdigit(resFromFile[index]))
	{
		number = number * 10 + (resFromFile[index++] - '0');
	}
	totalPoints = number;
...
bool checkWin(char& key,int score,int totalPoints,int caseOpt)
{
	...
	if (score >= totalPoints && caseOpt == 4)
	{
		clear_screen();
		return true;
	}//if won the game
